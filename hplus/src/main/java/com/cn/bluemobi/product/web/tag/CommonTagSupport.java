 ...
 * Version: V1.0 
 */
package com.cn.bluemobi.product.web.tag;
...
public class CommonTagSupport extends TagSupport {
	...
	/** 是否接受控制 */
	private boolean isAuth = TagConstantIF.IS_UNDER_CONTROL_NO;

	/** 事件ID */
	private String actionId = "";

	protected String addCommonItem() {
	...
	public String getLoginUserId() {
		...
		// 从会话中取得登录用户信息
		loginUser = SecurityUtils.getLoginUser(request);
...
/*	*//**
	...
	public boolean checkAuthByUrl(String href) {
		...
		href=href.indexOf(contextPath)>=0?href.substring(contextPath.length()):href;
		href=href.indexOf("?")>0?href.substring(0,href.lastIndexOf("?")):href;
		List<Menu> menuList=SessionMgr.getUserMenus(request);
		boolean flag=false;
		for(Menu menu:menuList){
			//System.out.println(href+":"+menu.getUrl()+":"+href.indexOf(menu.getUrl()));
			if(StringUtils.isNotBlank(menu.getUrl())&&menu.getUrl().indexOf(href)>=0){
				flag=true;
				break;
			}
		}
...
*/
	...
	public static void main(String[] args) {
		System.out.println("/admin/userManager/newUser.do".indexOf("userManager/newUser.do"));
	}
}
